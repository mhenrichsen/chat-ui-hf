				...
				<p>
					...
					{#if !assistant}
						...
						<a href="{base}/settings/assistants/{assistant._id}" class="hover:underline"
							>{model?.displayName}</a
						>{/if} <span class="max-sm:hidden">·</span><br class="sm:hidden" /> DanskGPT producerer ikke nødvendigvis sandfærdig information. - Inferns sponsoreret af <a href="https://capturi.com">Capturi</a>.
				</p>
				{#if messages.length}
					<button
